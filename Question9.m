...
varsemint=[6 7]; %variables de la semaine dont on requiert l'intégralité
for i=0:d.T-1 
    intvars(1,1+2*i:2+2*i)=varsemint+8*i;
end
options = optimoptions(@linprog, 'Algorithm', 'simplex');%Sol entière si elle existe
options = optimoptions('intlinprog', 'RootLPAlgorithm', 'dual-simplex');%Sol entière si elle existe
tab = reshape(intlinprog(C,intvars,Aineq,bineq,Aequal,bequal,lb,ub,options),8,15)';
tab(1:15,9)=d.demande;
tab
[x,cout] = intlinprog(C,intvars,Aineq,bineq,Aequal,bequal,lb,ub,options); %Ajout des salaires réguliers
